...
class Graph(object):
    ...
    def get_graph_patch(self, pose):
        ...
        #print ("memory size " , size(self.memory))
        print ("memory size " , self.memory.shape)
        print ("pose ", pose)
        print ("minx, miny", self.xMin, self.yMin)
    ...
    def get_shortest_path(self, pose, goal_pose):
        ...
        #print ("planned path by astar" , path)
        ...
        for ii in range(len(path) - 1):
            ...
            if weight == MAX_WEIGHT:
                print ("breaking out of the loop" , max_point)
                break
            max_point += 1
        '''
        #max_point = len(path)-1
        #path = path[:max_point]
